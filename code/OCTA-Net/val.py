...
def val_first_stage(best_thin, best_thick, best_fusion, viz, writer, dataloader, net,
    ...
    # thin_auc = round(visual_results(loss_dct["thin"], auc_dct["thin"], acc_dct["thin"], sen_dct["thin"], fdr_dct["thin"], spe_dct["thin"],
    #                  kappa_dct["thin"], gmean_dct["thin"], iou_dct["thin"], dice_dct["thin"], viz, writer, epoch, flag="_thin") + 1e-12, 4)
    ...
    checkpoint_path = os.path.join(models_dir, "{net}-{epoch}-{fusion_AUC}.pth")
