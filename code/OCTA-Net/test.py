...
def test_first_stage(dataloader, net, device, results_dir,
    ...
    #     fdr_dct["thin"], spe_dct["thin"], kappa_dct["thin"], gmean_dct["thin"], \
    #     iou_dct["thin"], dice_dct["thin"] = print_results(loss_dct["thin"], auc_dct["thin"], acc_dct["thin"], sen_dct["thin"], fdr_dct["thin"],
    ...
    #     fdr_dct["thick"], spe_dct["thick"], kappa_dct["thick"], gmean_dct["thick"], \
    #     iou_dct["thick"], dice_dct["thick"] = print_results(loss_dct["thick"], auc_dct["thick"], acc_dct["thick"], sen_dct["thick"], fdr_dct["thick"],
    #                                                         spe_dct["thick"], kappa_dct["thick"], gmean_dct["thick"], iou_dct["thick"], dice_dct["thick"])
    ...
    loss_dct["fusion"], auc_dct["fusion"], acc_dct["fusion"], sen_dct["fusion"], \
        ...
        iou_dct["fusion"], dice_dct["fusion"] = print_results(loss_dct["fusion"], auc_dct["fusion"], acc_dct["fusion"], sen_dct["fusion"],
                                                              fdr_dct["fusion"], spe_dct["fusion"], kappa_dct["fusion"], gmean_dct["fusion"], iou_dct["fusion"], dice_dct["fusion"])
...
def test_second_stage(dataloader, first_net_thick, first_net_thin, fusion_net, device, results_dir, criterion=None, isSave=True):
    ...
    with torch.no_grad():
        for sample in dataloader:
            ...
            # thin_gt = sample[2].to(device)
            # thick_gt = sample[3].to(device)
            # TODO: the output of these functions are tuple and cant be concatinated in the second stage.
            # TODO: Make sure the indices for the thick and thin are correct!
            thick_pred = first_net_thick(img)[0]
            thin_pred = first_net_thin(img)[1]
